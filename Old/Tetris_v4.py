...
def clear_rows(grid, locked):
    ...
    if inc > 0:
        # Remove and shift locked positions
        new_locked = {}
...
def draw_window(surface, grid, score=0, high_score=0, level=1, next_shape=None):
    ...
    # Reduce font size by two points (from 30 to 28)
...
def main(win):
    ...
    fall_time = 0
    fall_speed = 0.27  # Base fall speed
    level_time = 0
    score = 0
    level = 1
    lines_cleared = 0
    ...
    # Key press handling for continuous movement
    move_left_time = 0
    move_right_time = 0
    move_down_time = 0
    MOVE_INTERVAL_LEFT_RIGHT = 150  # milliseconds for 2x speed
    MOVE_INTERVAL_DOWN = 100  # milliseconds for 6x speed
    ...
    while run:
        ...
        if keys[pygame.K_LEFT]:
            if current_time - move_left_time > MOVE_INTERVAL_LEFT_RIGHT:
                current_piece.x -= 1
                if not valid_space(current_piece, grid):
                    current_piece.x += 1
                else:
        ...
        if keys[pygame.K_RIGHT]:
            if current_time - move_right_time > MOVE_INTERVAL_LEFT_RIGHT:
                current_piece.x += 1
                if not valid_space(current_piece, grid):
                    current_piece.x -= 1
                else:
        ...
        if keys[pygame.K_DOWN]:
            if current_time - move_down_time > MOVE_INTERVAL_DOWN:
                current_piece.y += 1
                if not valid_space(current_piece, grid):
                    current_piece.y -= 1
                else:
        ...
        if check_lost(locked_positions):
            ...
            draw_text_middle(win, "GAME OVER", 78, WHITE)  # Reduced font size from 80 to 78
            pygame.display.update()
...
def main_menu(win):
    ...
    while run:
        win.fill(BLACK)
        draw_text_middle(win, "Press Any Key To Play", 58, WHITE)  # Reduced font size from 60 to 58
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
